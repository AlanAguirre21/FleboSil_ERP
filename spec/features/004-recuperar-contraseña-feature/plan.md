...
5. Frontend — crear `frontend/src/pages/RecuperarContrasena/RecuperarContrasena.tsx`: paso 1 (input de correo + botón enviar), paso 2 (input de 6 dígitos + botón verificar + opción de reenviar).
6. Frontend — crear `frontend/src/api/auth.ts` (extendiendo el ya creado en Login) con las funciones `solicitarRecuperacion()` y `verificarCodigo()`.
