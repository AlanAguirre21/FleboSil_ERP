...
- [ ] Crear `frontend/src/api/inventario.js` con funciones de consulta.
- [ ] Crear hooks `useInventario.js` y `useMovimientosInventario.js`.
- [ ] Crear `frontend/src/pages/Inventario/Inventario.jsx` con selector de tipo, selector de sucursal y tabla de stock con indicador de alerta.
- [ ] Implementar sección de historial con filtros combinados, sin botones de acción.
- [ ] Agregar la ruta `/inventario` en `App.jsx` y el módulo al diccionario "módulo → roles permitidos" (sin restricción de rol).
