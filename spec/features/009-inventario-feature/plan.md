...
9. Frontend — crear `frontend/src/api/inventario.js` con las funciones de consulta (stock por tipo+sucursal, historial con filtros).
10. Frontend — crear `frontend/src/hooks/useInventario.js` y `useMovimientosInventario.js` con React Query.
11. Frontend — crear `frontend/src/pages/Inventario/Inventario.jsx`: selector de tipo (producto/materia prima), selector de sucursal, tabla de stock con indicador visual de alerta (usando `--color-advertencia` de `variables.css`).
...
13. Frontend — agregar la ruta `/inventario` en `App.jsx`, sin restricción de rol.
