...
3. Frontend — crear `frontend/src/pages/CambiarContrasena/CambiarContrasena.tsx`, leyendo el estado de navegación (correo/token) pasado desde la feature 004; si no existe, redirige a `/recuperar-contrasena`.
4. Frontend — crear `frontend/src/pages/CambiarContrasena/CambiarContrasena.module.css`, diseño particular sin `MainLayout`.
5. Frontend — extender `frontend/src/api/auth.ts` con `cambiarContrasena()`.
