...
- [x] Esta pantalla solo es accesible tras una verificación de código exitosa desde `004 · Recuperar contraseña` — no es alcanzable navegando directo por URL sin ese contexto previo.
- [x] El formulario solicita nueva contraseña y confirmación de nueva contraseña.
- [x] Si ambas contraseñas no coinciden, se muestra un error claro antes de intentar guardar.
- [x] La nueva contraseña debe cumplir requisitos mínimos de seguridad (longitud mínima, no ser idéntica a la anterior); si no los cumple, se informa específicamente cuál requisito falta.
- [x] Al guardar exitosamente, la contraseña se almacena únicamente como hash, nunca en texto plano.
- [x] Al completar el cambio, el usuario inicia sesión automáticamente y es redirigido al Dashboard principal.
- [x] El código de recuperación usado para llegar a esta pantalla queda invalidado permanentemente tras el cambio exitoso, sin importar que aún esté dentro de su ventana de expiración.
- [x] Si el contexto de verificación expira o es inválido (ej. el usuario llegó aquí sin pasar por el paso anterior), se redirige de vuelta a `004 · Recuperar contraseña` con un mensaje explicativo.
