...
- [ ] Crear `frontend/src/api/contabilidad.ts`.
- [ ] Crear hooks `useCuentasContables.ts`, `useLibroDiario.ts`, `useBalanceComprobacion.ts`.
- [ ] Crear `frontend/src/pages/Contabilidad/Contabilidad.tsx` con selector de secciones.
- [ ] Implementar vista expandible de asientos en el libro diario.
- [ ] Implementar botón de exportación en ambas vistas.
- [ ] Agregar la ruta `/contabilidad` en `App.tsx` y el módulo al diccionario "módulo → roles permitidos" (solo admin).
