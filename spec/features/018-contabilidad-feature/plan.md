...
12. Frontend — crear `frontend/src/api/contabilidad.ts` con funciones CRUD de cuentas, consulta de libro diario, balance, y exportación.
13. Frontend — crear `frontend/src/hooks/useCuentasContables.ts`, `useLibroDiario.ts`, `useBalanceComprobacion.ts`.
14. Frontend — crear `frontend/src/pages/Contabilidad/Contabilidad.tsx`: selector entre "Catálogo de cuentas", "Libro diario" y "Balance de comprobación".
...
17. Frontend — agregar la ruta `/contabilidad` en `App.tsx`, protegida exclusivamente para admin.
