...
10. Frontend — crear `frontend/src/api/facturacion.ts` con funciones de listado, generación, cancelación, descarga y registro de complemento de pago.
11. Frontend — crear `frontend/src/hooks/useFacturas.ts` con React Query.
12. Frontend — modificar `frontend/src/pages/Ventas/DetalleVenta.tsx` (de `011`) para habilitar el botón "Generar factura" según los datos fiscales del cliente.
13. Frontend — crear `frontend/src/pages/Facturacion/Facturacion.tsx`: lista filtrable por estado/fecha/cliente, con acciones de descarga y cancelación según el estado de cada factura.
...
16. Frontend — agregar la ruta `/facturacion` en `App.tsx`, sin restricción de rol.
