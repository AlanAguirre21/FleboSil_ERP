...
9. Frontend — crear `frontend/src/api/caja.ts` con funciones de listado (con filtros) y creación de movimiento manual.
10. Frontend — crear `frontend/src/hooks/useMovimientosCaja.ts` con React Query.
11. Frontend — crear `frontend/src/pages/Caja/Caja.tsx`: saldo actual destacado en la parte superior, tabla de movimientos filtrable por fecha/tipo/motivo, botón "Registrar ingreso/retiro" que abre un formulario en `Modal`.
...
14. Frontend — agregar la ruta `/caja` en `App.tsx`, protegida exclusivamente para admin.
