...
10. Frontend — crear `frontend/src/api/compras.js` con funciones de listado, creación, detalle, recibir y cancelar.
11. Frontend — crear `frontend/src/hooks/useCompras.js` con React Query, incluyendo invalidación de cache de `useInventario` al recibir/cancelar una compra (para que la feature `009` refleje el cambio sin recargar).
12. Frontend — crear `frontend/src/pages/Compras/Compras.jsx`: lista filtrable por proveedor/fecha/estado, reutilizando `Tabla`.
13. Frontend — crear `frontend/src/pages/Compras/NuevaCompra.jsx`: selector de proveedor, selector de sucursal destino, buscador de ítems (producto o materia prima) que agrega líneas dinámicamente, cálculo de total en vivo.
14. Frontend — crear `frontend/src/pages/Compras/DetalleCompra.jsx`: muestra líneas, totales, y botones de acción ("Marcar como recibida", "Cancelar") según el estado actual.
15. Frontend — agregar la ruta `/compras` en `App.jsx`, sin restricción de rol.
