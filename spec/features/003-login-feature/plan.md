...
4. Frontend — crear `frontend/src/context/AuthContext.tsx`, exponiendo `login()`, `logout()`, y el estado de sesión actual a toda la aplicación.
5. Frontend — crear `frontend/src/pages/Login/Login.tsx` con el formulario (correo, contraseña), validación básica de campos vacíos, y llamada a `AuthContext.login()`.
6. Frontend — crear `frontend/src/pages/Login/Login.module.css` con diseño particular (no reutiliza `MainLayout`).
7. Frontend — en `frontend/src/api/auth.ts`, centralizar la llamada `POST /api/auth/login/` y el manejo de la respuesta (token + datos básicos de usuario).
8. Frontend — configurar interceptor de Axios en `frontend/src/api/client.ts` (nombre real del archivo — este plan lo llamaba `axiosClient.ts`, desajuste con el código corregido aquí) para adjuntar el JWT automáticamente en cada petición subsecuente.
9. Frontend — configurar rutas protegidas en `frontend/src/App.tsx`: si no hay sesión válida, cualquier ruta protegida redirige a `/login`.
10. Frontend — agregar enlace a "Recuperar contraseña" en `Login.tsx`, apuntando a la ruta de la feature `004`.
...
- **Expiración de token durante una operación larga (ej. registrar una venta con muchos productos), causando pérdida de datos no guardados** — mitigación: implementar refresh automático de token en el interceptor de Axios (`client.ts`) antes de que expire, de forma transparente para el usuario.
