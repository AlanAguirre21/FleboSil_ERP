...
- [ ] El formulario solicita correo electrónico y contraseña, ambos obligatorios.
- [ ] Si las credenciales son correctas, el usuario es redirigido al Dashboard y recibe un token JWT válido almacenado de forma seguraS en el cliente.
- [ ] Si las credenciales son incorrectas, se muestra un mensaje de error genérico ("correo o contraseña incorrectos"), sin indicar cuál de los dos campos falló — para no dar pistas útiles a un intento de acceso no autorizado.
- [ ] Si el usuario está marcado como `activo = false`, el acceso se rechaza con un mensaje claro, aunque la contraseña sea correcta.
- [ ] Existe un enlace visible a "Recuperar contraseña".
- [ ] No existe ningún enlace ni formulario de autorregistro en esta pantalla.
- [ ] Las contraseñas nunca se transmiten ni se almacenan en texto plano — se validan mediante hash tanto en el envío (HTTPS) como en el almacenamiento (hashing de Django).
- [ ] Tras múltiples intentos fallidos consecutivos (a definir, ej. 5), el sistema aplica una restricción temporal antes de permitir un nuevo intento, para mitigar ataques de fuerza bruta.
- [ ] El formulario es responsive y utilizable correctamente en pantallas pequeñas.
