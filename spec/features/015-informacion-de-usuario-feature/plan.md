...
5. Frontend — crear `frontend/src/pages/InformacionUsuario/InformacionUsuario.jsx`, precargando los datos desde `useUsuarioActual` (ya existente desde `001`).
...
8. Frontend — extender `frontend/src/api/personas.js` (o crear `frontend/src/api/usuarioActual.js` si se prefiere separar del CRUD administrativo) con la función `actualizarMiInformacion()`.
...
10. Frontend — agregar la ruta `/mi-informacion` (o similar) en `App.jsx`, accesible desde el menú de usuario del Header.
