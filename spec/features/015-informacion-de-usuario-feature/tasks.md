...
- [ ] Crear `frontend/src/pages/InformacionUsuario/InformacionUsuario.tsx`, precargando datos desde `useUsuarioActual`.
...
- [ ] Extender `frontend/src/api/` (`.ts`) con `actualizarMiInformacion()`.
- [ ] Invalidar la cache de `useUsuarioActual` tras actualización exitosa.
- [ ] Agregar la ruta `/mi-informacion` en `App.tsx`, enlazada desde el menú de usuario del Header.
