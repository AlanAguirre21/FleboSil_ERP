...
- [ ] Crear `frontend/src/api/personas.ts` con funciones CRUD de los cuatro tipos.
- [ ] Crear hooks `useClientes.ts`, `useProveedores.ts`, `useEmpleados.ts`, `useUsuarios.ts`.
- [ ] Crear `frontend/src/pages/Personas/Personas.tsx` con selector de pestañas.
...
- [ ] Agregar la ruta `/personas` en `App.tsx` y el módulo al diccionario "módulo → roles permitidos".
