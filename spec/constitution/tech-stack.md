...
- **Lenguaje:** Python 3.12 (backend) + JavaScript ES2022 (frontend)
- **Framework / runtime:** Django 5.x + Django REST Framework (backend) / React 18 con Vite + CSS Modules (frontend)
...
- `frontend/src/api/` — una función por endpoint, agrupada por módulo; ningún componente llama `axios` directo, siempre pasa por aquí.
- `frontend/src/hooks/` — hooks de React Query por entidad; centraliza cache y estados de carga/error.
- `frontend/src/styles/variables.css` — variables CSS globales (colores de marca, colores de estado, tipografía, breakpoints); todo archivo `.module.css` las consume vía `var(--nombre)`, nunca hardcodea un color directo.
- `frontend/src/components/*/Componente.module.css` — un archivo de estilos por componente, junto a su `.jsx`; nunca estilos inline ni CSS mezclado dentro del JSX.
...
- `npm run lint` — lint de JavaScript/React (ESLint).
...
- `snake_case` para modelos y campos de Django; `camelCase` para variables y funciones en React/JavaScript.
- Estilos: un archivo `Componente.module.css` por componente, importado como `import styles from './Componente.module.css'` y aplicado vía `className={styles.boton}`; nunca CSS inline ni mezclado en el JSX.
...
- **Paleta de color:** definida como variables CSS globales en `frontend/src/styles/variables.css`:
...
- **Colores de estado (independientes de la paleta de marca), también como variables globales:**
...
- **Tipografía:** fuente del sistema, definida como `--font-family-base` en `variables.css`.
- **Responsive:** breakpoints definidos como variables (`--breakpoint-sm`, `--breakpoint-md`, `--breakpoint-lg`); sidebar colapsable por debajo de `--breakpoint-md`.
